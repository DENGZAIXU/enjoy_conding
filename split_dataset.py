...
def setup_files(class_dir, seed):
    ...
    random.seed(seed)
 # 修改 切割 image 和 annotation
    files = list_files(os.path.join(class_dir,"annotation"))
...
def split_files(files, split_train, split_val, use_test):
    ...
    li = [(files_train, 'train'), (files_val, 'validation')]
...
if __name__ == '__main__':
    input = "/home/algoteam2/xu/dataset/data/"
    output = "/home/algoteam2/xu/dataset"
    fixed(input, output, seed=1337, fixed=(100,100), oversample=False)

   # 固定数量分割 分割 训练集/测试集/验证集 fixed = (100, 100)
   #  只分割训练集和验证集 fixed = 100
   #  input = r"C:\Users\Xu\Desktop\data\dataset"

    # fixed(input, output="output", seed=1337, fixed=(100, 100), oversample=False)
    # ratio(input, output="output", seed=1337, ratio=(.8, .1, .1))
