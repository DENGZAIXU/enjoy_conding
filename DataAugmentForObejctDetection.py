...
if __name__ == '__main__':
    ...
    source_pic_root_path = './rotate'
    source_xml_root_path = './xml_rot'
    savepath = './cao/'
    out_root_path = './fuck'
    cxk = 4000
    for parent, _, files in os.walk(source_pic_root_path):
        for file in files:
            ...
            while cnt < need_aug_num:
                ...
                idlei = [coord[-1] for coord in coords]
                #print(idlei)
                #print(coords)
                ...
                res = []
                for Q in range(len(idlei)):   
                    ns = [int(auged_bboxes[Q][0]),int(auged_bboxes[Q][1]),int(auged_bboxes[Q][2]),int(auged_bboxes[Q][3]),idlei[Q]]
                    res.append(ns)
                print(res)
                cnt += 1
                cxk += 1
                cv2.imwrite(savepath+file[:-4]+'_'+str(cxk)+'.png',auged_img)
                fuck = cv2.imread(savepath+file[:-4]+'_'+str(cxk)+'.png')
                img_size = fuck.shape
                generate_xml(file[:-4]+'_'+str(cxk)+'.png',res,img_size,out_root_path)
